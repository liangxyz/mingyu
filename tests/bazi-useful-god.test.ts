...
test('乙日午月上半月若柱多金水，调候应改以丙火为先，不应仍一概先取癸水', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wu-month-yi-mangzhong-metal-water-fire-first'),
  );
});
...
test('己日夏月无癸而壬透时，不应退回普通扶抑，应允许壬水权代润土', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'si-wu-wei-month-ji-no-gui-ren-allowed'),
  );
});
...
test('丁日卯月庚乙俱透时，应标记贪合致贫，不应仍按庚甲成格路线判断', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'mao-month-ding-geng-yi-greedy-combine'),
  );
});
...
test('丙日酉月支成金局而无辛出干时，应标记朱门饿莩，不应误作从才富贵', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'you-month-bing-metal-formation-no-xin'),
  );
});
...
test('丙日酉月支成金局且辛透又不见比劫时，应标记从才格反主富贵', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'you-month-bing-metal-formation-xin-follow-wealth',
    ),
  );
});
...
test('丙日酉月壬透而土重时，应降为假作斯文，不应仍按一壬高透富贵双全上断', () => {
  ...
  assert.ok(!result.strategyTrace?.includes('成格层次:登科及第，富贵双全'));
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'you-month-bing-wu-heavy-false-scholar'),
  );
});
...
test('丙日戌月甲藏壬透而无庚破甲时，应标记可许秀才，不应误判为甲壬两透富贵非凡', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'xu-month-bing-hidden-jia-ren-no-geng'),
  );
});
...
test('辛日辰月壬透甲藏时，应标记廪贡不失，不应与壬甲两透或平常格混同', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'chen-month-xin-ren-visible-jia-hidden'),
  );
});
...
test('辛日辰月月时皆丙而又得癸水制丙时，应标记可许采芹，不应仍按争合风流处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'chen-month-xin-double-bing-with-gui-scholarly',
    ),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'chen-month-xin-double-bing-argue-combine'),
  );
});
...
test('辛日辰月支坐亥子之乡而又见申时，应标记高增禄位，不应仍按普通辰月平断', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'chen-month-xin-hai-zi-land-with-shen-rank'),
  );
});
...
test('辛日辰月戊土出干制水而不见甲乙时，应标记清闲之人，不应误抬到壬甲富贵层次', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'chen-month-xin-wu-control-water-no-jia-yi-leisure',
    ),
  );
});
...
test('辛日辰月四支齐见四库而甲不透时，应标记愚顽之辈，不应仍按平常格处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'chen-month-xin-four-storage-no-jia-dull'),
  );
});
...
test('辛日辰月火多而无壬癸透制时，应标记主作缁衣，不应仍按普通春金调候收束', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'chen-month-xin-fire-many-no-water-monastic'),
  );
});
...
test('辛日辰月火多而癸透时，应标记见癸可解，不应仍按缁衣处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'chen-month-xin-fire-many-with-gui-relief'),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'chen-month-xin-fire-many-no-water-monastic'),
  );
...
test('辛日辰月比劫重重而壬癸浅弱时，应标记主夭，不应仍按普通有根身旺处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'chen-month-xin-companions-heavy-water-weak-early-loss',
    ),
  );
});
...
test('辛日辰月比劫重重而甲透且不见庚时，应标记则贵，不应仍按主夭处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'chen-month-xin-companions-heavy-water-weak-jia-noble',
    ),
  );
  assert.ok(
    !result.matchedRules?.some(
...
test('辛日戌月壬透甲藏而又见戊时，应只作平人，不应误抬到桃洞或异途层次', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'xu-month-xin-ren-visible-jia-hidden-with-wu'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'xu-month-xin-ren-jia-all'));
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'xu-month-xin-jia-visible-ren-hidden'),
  );
});
...
test('辛日戌月土厚甲不透而壬出时，应断富而可求，不应误入平人层次', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'xu-month-xin-heavy-earth-ren-visible-jia-hidden-rich',
    ),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'xu-month-xin-ren-visible-jia-hidden-with-wu'),
...
test('辛日戌月土多无壬甲而时月多透丙辛时，应断略贵，不应仍退回常格', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'xu-month-xin-no-ren-jia-bing-xin-slight-noble',
    ),
  );
  assert.ok(
    !result.matchedRules?.some(
      (rule) => rule.id === 'xu-month-xin-no-ren-jia-bing-xin-with-chen-glory',
    ),
  );
});
...
test('辛日戌月木多土厚而全无壬癸时，应断常人，不应误抬到有水层次', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'xu-month-xin-wood-many-earth-thick-no-water-ordinary',
    ),
  );
  assert.ok(
    !result.matchedRules?.some(
      (rule) => rule.id === 'xu-month-xin-wood-earth-double-gui-rich-hardship',
    ),
  );
});
...
test('辛日戌月己透无壬有癸且己不多时，应断衣衿之贵，不应误落浊富', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'xu-month-xin-ji-visible-gui-no-ren-scholarly'),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'xu-month-xin-many-ji-gui-no-ren-cloudy-rich'),
  );
});
...
test('辛日戌月己多无壬有癸时，应断浊富，不应仍按衣衿之贵', () => {
  ...
  assert.ok(
    !result.matchedRules?.some(
      (rule) => rule.id === 'xu-month-xin-ji-visible-gui-no-ren-scholarly',
    ),
  );
});
...
test('辛日亥月丙透壬藏时，应标记采芹之造，不应误判为两透或寒湿偏枯', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'hai-month-xin-bing-visible-ren-hidden'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'hai-month-xin-ren-bing-all'));
});
...
test('辛日亥月壬透丙藏时，应标记富有千金，不应误落到丙透壬藏或壬丙两透', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'hai-month-xin-ren-visible-bing-hidden'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'hai-month-xin-ren-bing-all'));
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'hai-month-xin-bing-visible-ren-hidden'),
...
test('辛日亥月己多有戊且仍有壬时，应标记不过诚实之人，不应误抬到成名层次', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'hai-month-xin-ji-many-with-wu-honest'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'hai-month-xin-wu-many-ren-few-fame'));
});
...
test('辛日子月壬藏丙透时，应标记一榜堪图，不应误抬到壬丙两透', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'zi-month-xin-ren-hidden-bing-visible'),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'zi-month-xin-ren-bing-all-no-wu-gui'),
  );
});
...
test('辛日子月癸出而丙透、却无壬时，应标记冻金困丙，不应误按衣锦腰金或一榜堪图处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'zi-month-xin-gui-visible-bing-visible-no-ren'),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'zi-month-xin-ren-bing-all-no-wu-gui'),
  );
...
test('辛日子月壬多有戊而丙甲出干时，应标记青云之客，不应与壬多寒儒同断', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'zi-month-xin-ren-many-with-wu-bing-jia'),
  );
...
test('辛日子月壬多而甲乙重重、又无丙时，应标记贫寒，不应仍只按寒儒处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'zi-month-xin-ren-many-jia-yi-heavy-no-bing'),
  );
});
...
test('辛日子月支成水局而癸透、又有二戊制水时，应标记富贵恩荣，不应仍按常人处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'zi-month-xin-water-formation-gui-two-wu'),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'zi-month-xin-water-formation-gui-no-wu'),
  );
});
...
test('辛日子月亥子丑全而无庚辛、反见甲乙且无戊丙时，应标记僧道，不应误按润下格富贵处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'zi-month-xin-run-down-no-metal-monastic'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'zi-month-xin-run-down-prosper'));
});
...
test('辛日子月支成木局而丁戊并见时，应标记功名特达，不应仍按普通冬金木旺受克处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'zi-month-xin-wood-formation-ding-wu-merit'),
  );
});
...
test('辛日丑月丙多而无壬、有癸时，应标记市中贸易，不应仍只按有丙无壬富真贵假处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'chou-month-xin-many-bing-no-ren-gui-trade'),
  );
});
...
test('辛日丑月水多而戊己出干、又见丙丁时，应标记衣食充盈一生安乐，不应仍按寒湿偏枯处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'chou-month-xin-water-many-earth-fire-peace'),
  );
});
...
test('辛日寅月己壬两透且支见庚制甲时，应标记科甲定然，不应仍按普通春金调候处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'yin-month-xin-ji-ren-visible-geng-hidden-jia-control',
    ),
  );
});
...
test('辛日寅月己透而支中有甲时，应标记异路恩荣，不应误抬到己壬两透科甲', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'yin-month-xin-ji-visible-jia-hidden-alt-grace',
    ),
  );
  assert.ok(
    !result.matchedRules?.some(
...
test('辛日寅月见壬而全无己庚时，应标记贫贱之徒，不应误把壬水当作得用', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'yin-month-xin-ren-visible-no-ji-geng-poor'),
  );
});
...
test('辛日寅月壬透而庚尚存、己不透时，应标记富贵难全，不应误落贫贱', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'yin-month-xin-ren-visible-geng-no-ji-incomplete',
    ),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'yin-month-xin-ren-visible-no-ji-geng-poor'),
...
test('辛日寅月支成火局而壬透兼有己土时，应标记寻常之人，不应误抬到破局显达', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'yin-month-xin-fire-formation-ren-only-ordinary',
    ),
  );
});
...
test('辛日寅月支成火局而庚壬两透时，应标记显达，不应仍按火局壬透寻常处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'yin-month-xin-fire-formation-geng-ren-all-distinguished',
    ),
  );
  assert.ok(
    !result.matchedRules?.some(
...
test('辛日寅月支成水局而仅月令暗藏丙、不见丙透时，应标记金弱沉寒平常之士', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'yin-month-xin-water-formation-no-bing-ordinary',
    ),
  );
});
...
test('辛日寅月支成水局而丙火透出时，应标记反主富贵，不应仍按沉寒平常处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'yin-month-xin-water-formation-bing-visible-rich',
    ),
  );
  assert.ok(
    !result.matchedRules?.some(
...
test('辛日寅月逢卯日子时，应标记朝阳格象，不应仍只按普通寅月辛金判断', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'yin-month-xin-mao-day-zi-hour-chaoyang'),
  );
});
...
test('辛日卯月壬丙齐透时，应标记大富大贵，不应仍只按壬甲贵显层次', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-ren-bing-all-great-wealth'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-ren-jia-all-noble'));
});
...
test('辛日卯月壬坐亥支且天干不见戊己时，应标记家亦小康，不应把别柱亥支误作壬坐亥', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-ren-sits-hai-no-earth-well-off'),
  );
});
...
test('辛日卯月虽见壬但亥支在别柱时，不应误判为壬坐亥支家亦小康', () => {
  ...
  assert.ok(
    !result.matchedRules?.some(
      (rule) => rule.id === 'mao-month-xin-ren-sits-hai-no-earth-well-off',
    ),
  );
});
...
test('辛日卯月壬不透而得申中暗壬时，应标记异途名望，不应退回无壬常人', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-shen-hidden-ren-alt-fame'),
  );
...
test('辛日卯月壬戊透而甲仅暗藏不透时，仍应标记平常，不应误把暗藏甲当成甲出干', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-ren-wu-visible-no-jia-ordinary'),
  );
});
...
test('辛日卯月壬戊乙透而甲仅暗藏不透时，应标记假名假利，不应因暗藏甲而漏判', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-ren-wu-with-yi-scholarly-false'),
  );
  assert.ok(
    !result.matchedRules?.some(
      (rule) => rule.id === 'mao-month-xin-ren-wu-visible-no-jia-ordinary',
    ),
  );
});
...
test('辛日卯月壬水重重而无戊时，应标记略有衣食全无作为，不应仍按普通有壬论', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-ren-flood-no-wu-meager'),
  );
});
...
test('辛日卯月壬水重重而得戊时，应标记得戊反吉，不应仍按汪洋无为处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-ren-flood-with-wu-auspicious'),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-ren-flood-no-wu-meager'),
  );
...
test('辛日卯月支成木局而庚仅暗藏时，仍应按有庚富贵，不应误限为庚必须透干', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-wood-formation-with-geng-rich'),
  );
});
...
test('辛日卯月支成木局而全无庚金时，应标记平人，不应误判为木局有庚富贵', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'mao-month-xin-wood-formation-no-geng-ordinary',
    ),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-wood-formation-with-geng-rich'),
...
test('辛日卯月支成火局而二壬透出制火时，应标记富贵反奇，不应把藏壬混作出壬数量', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'mao-month-xin-fire-formation-double-ren-marvel',
    ),
  );
});
...
test('辛日卯月支成火局而仅一壬透、一壬暗藏时，不应误判为二壬出制富贵反奇', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-fire-formation-base-low'),
  );
  assert.ok(
    !result.matchedRules?.some(
      (rule) => rule.id === 'mao-month-xin-fire-formation-double-ren-marvel',
...
test('辛日卯月壬水纯一而不见丙甲戊己时，应标记显达，不应退回一般平常层次', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-pure-ren-no-bing-prominent'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'mao-month-xin-no-ren-ordinary'));
});
...
test('辛日巳月支成金局而水透木制戊时，应标记一清澈底科甲功名，不应仍只按普通夏辛喜水处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'si-month-xin-metal-formation-water-wood-clarity',
    ),
  );
});
...
test('辛日巳月壬藏亥中而甲透时，应标记衣禄可求，不应仍按上达或常人处理', () => {
  ...
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'si-month-xin-ren-hai-hidden-with-wu'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'si-month-xin-ren-hai-hidden-no-wu'));
});
...
test('辛日巳月壬癸皆藏且戊己亦藏时，应标记略富，不应误作壬癸全无', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'si-month-xin-ren-gui-hidden-wu-ji-hidden'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'si-month-xin-no-ren-gui-jia'));
});
...
test('辛日巳月支成火局而得水制时，应标记有制则吉，不应退成无水取土或鳏独', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'si-month-xin-fire-formation-water-control'),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'si-month-xin-fire-formation-no-water-earth'),
  );
});
...
test('辛日午月壬己两透而支见癸水不冲时，应标记显达，不应仍退回泛化己壬并用', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wu-month-xin-ren-ji-visible-hidden-gui'),
  );
});
...
test('辛日午月壬透而己只藏支、支见癸水不冲时，应标记廪贡，不应误拔到壬己两透显达', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wu-month-xin-ren-visible-ji-hidden-gui-page'),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'wu-month-xin-ren-ji-visible-hidden-gui'),
  );
...
test('辛日午月无壬而己透时，应标记异途，不应仍按壬己并用显达处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wu-month-xin-no-ren-ji-visible-alt-path'),
  );
});
...
test('辛日午月无壬而癸透、且仍有己土承接时，应标记癸水权代，不应继续停留在通用夏金先水', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wu-month-xin-no-ren-gui-weak-substitute'),
  );
});
...
test('辛日午月无壬而癸戊并见，若再有一二重辛金时，应标记僧道而不孤，不应仍按纯僧道孤寒处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wu-month-xin-no-ren-gui-wu-with-companion'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'wu-month-xin-gui-geng-no-ren-grace'));
});
...
test('辛日午月支成火局而壬透时，应标记破火生员，不应把癸水权代误作足用', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wu-month-xin-fire-formation-ren-break-fire'),
  );
});
...
test('辛日午月支成火局而重见癸水、却无壬透时，应标记癸水亦不济，不应仍按癸水权代看待', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wu-month-xin-fire-formation-gui-heavy-no-ren'),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'wu-month-xin-no-ren-gui-weak-substitute'),
  );
...
test('辛日午月水土并重而甲透时，应标记见甲方妙，不应仍只停留在泛化水土并见', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wu-month-xin-water-earth-heavy-see-jia'),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'wu-month-xin-ren-ji-visible-hidden-gui'),
  );
...
test('辛日午月木火过盛而别无金水时，应标记逢金水运反败，不应仍只停留在通用夏金先水', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'wu-month-xin-wood-fire-heavy-no-metal-water-warning',
    ),
  );
});
...
test('辛日未月丁乙虽透但不见壬时，不应误按丁乙庚壬显贵规则处理', () => {
  ...
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'wei-month-xin-ding-yi-with-ren-geng'),
  );
});
...
test('辛日未月戊土出干而不见甲木制伏时，应标记戊出破局，不应仍按壬庚得用宽泛上断', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wei-month-xin-wu-visible-no-jia-break'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'wei-month-xin-ren-geng-no-wu'));
});
...
test('辛日未月一壬一己而见庚无甲时，应标记方妙，不应仍停留在泛化壬庚两透', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'wei-month-xin-single-ren-single-ji-geng-no-jia',
    ),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'wei-month-xin-ren-geng-no-wu'));
});
...
test('辛日未月支成木局而壬透见庚时，应标记可云富贵，不应仍停留在普通取水层次', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'wei-month-xin-wood-formation-ren-visible-geng-total',
    ),
  );
});
...
test('辛日申月壬居申中而全局不另见戊时，应标记官清不富，不应混入土重常人格', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'shen-month-xin-ren-in-shen-no-extra-wu'),
  );
...
test('辛日申月金多而壬透、一戊得甲制时，应标记自然富贵，不应仍停留在官清不富', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'shen-month-xin-metal-rich-ren-jia-single-wu'),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'shen-month-xin-ren-in-shen-no-extra-wu'),
  );
...
test('辛日申月干支水多、重见戊土而得火生时，应标记福寿之造，不应仍按普通土重处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'shen-month-xin-water-rich-wu-supported-longevity',
    ),
  );
});
...
test('辛日申月金多水浅而壬透不过多时，应标记体全之象，仍以壬为尊，不应只停留在泛化秋金喜水', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'shen-month-xin-metal-many-shallow-water-balance',
    ),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'shen-month-xin-gui-only-not-usable'));
  assert.ok(
...
test('辛日酉月比肩一二而壬甲各一、无庚时，应标记亦有恩荣，不应误落土厚埋金', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'you-month-xin-ren-jia-single-no-geng'),
  );
});
...
test('辛日酉月一壬而甲多，但得庚制甲时，应转为仁义，不应仍按奸诈处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'you-month-xin-ren-many-jia-with-geng'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'you-month-xin-ren-many-jia-no-geng'));
});
...
test('辛日酉月三辛一壬、甲多而庚透且不见丁时，应提升为大富贵，不应仍只按仁义处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'you-month-xin-three-xin-single-ren-many-jia-geng-rich',
    ),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'you-month-xin-ren-many-jia-with-geng'),
...
test('辛日酉月三辛一壬、甲多而庚透但再见丁时，应降为风雅清高，不应仍按大富贵处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'you-month-xin-three-xin-single-ren-many-jia-geng-ding-refined',
    ),
  );
  assert.ok(
    !result.matchedRules?.some(
...
test('辛日酉月二三比肩、一壬而戊土多见又无甲时，应标记愚懦，不应仍按富中取贵或恩荣', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'you-month-xin-soil-bury-metal-no-jia'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'you-month-xin-pure-xin-single-ren'));
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'you-month-xin-ren-jia-single-no-geng'),
...
test('辛日酉月土厚埋金而甲木透出时，应标记创立之人，不应仍按愚懦处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'you-month-xin-soil-bury-metal-with-jia'),
  );
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'you-month-xin-soil-bury-metal-no-jia'),
  );
...
test('辛日酉月支成金局且无壬淘洗时，应明确转取丁火总纲，不应仍执壬水为先', () => {
  ...
  assert.ok(
    !result.matchedRules?.some((rule) => rule.id === 'you-month-xin-metal-formation-ren-high'),
  );
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'you-month-xin-metal-formation-no-ren-with-ding',
    ),
  );
});
...
test('辛日酉月支成金局而无壬但已见丁时，应标记得丁锻炼，不应落入无丁凶顽', () => {
  ...
  assert.ok(
    !result.matchedRules?.some(
      (rule) => rule.id === 'you-month-xin-metal-formation-no-ren-no-ding',
    ),
  );
});
...
test('辛日酉月支成金局而无壬无丁时，应标记凶顽无赖，不应只停留在宜用丁的抽象提示', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'you-month-xin-metal-formation-no-ren-no-ding'),
  );
});
...
test('辛日酉月白虎格若丙火透出时，应降为平庸，不应继续按白虎格高断', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'you-month-xin-white-tiger-with-fire-ordinary'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'you-month-xin-white-tiger'));
});
...
test('辛日酉月己土透干而地支见庚甲时，应标记一生安闲，不应仍按僧道或通用秋金处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'you-month-xin-ji-with-geng-jia-hidden-leisure',
    ),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'you-month-xin-pure-ji-monastic'));
});
...
test('辛日酉月一派壬水但地支独见一戊止流时，应标记才略艺术，不应仍按奔波贫苦处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'you-month-xin-water-flood-hidden-wu-artistry'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'you-month-xin-water-flood-no-wu'));
});
...
test('辛日酉月辛日得戊子时又成巳酉丑全、庚辛并见时，应提升为位重权高', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'you-month-xin-wuzi-chaoyang-authority'),
  );
  assert.ok(!result.matchedRules?.some((rule) => rule.id === 'you-month-xin-wuzi-chaoyang'));
});
...
test('戊日寅月木多、无庚且无比印时，应标记难作从杀，不应误按可从之局处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'yin-mao-month-wu-no-geng-no-resource-follow-kill-fail',
    ),
  );
});
...
test('己日卯月木势偏盛且明暗都无比印时，应按从杀者贵处理', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'mao-month-ji-follow-kill-no-resource'),
  );
});
...
test('丁日子月水多癸旺且金无比印时，应标记弃命从杀而非普通冬丁调候', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'zi-month-ding-follow-kill-water-prosper'),
  );
});
...
test('丁日子月水多癸旺但再见丁比透干时，应降为常人，不应仍按弃命从杀论', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'zi-month-ding-follow-kill-broken-by-companion',
    ),
  );
  assert.ok(!result.strategyTrace?.includes('成格层次:弃命从杀，异途功名'));
});
...
test('癸日丑月癸己会党且年透丁火、夜生时，应提升为雪后灯光之贵，不应仍按普通冬水调候', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'chou-month-gui-gui-ji-party-night-ding'),
  );
});
...
test('癸日丑月支成水局而无丙火时，应标记四海为家一生劳苦，不应仍按普通冬水调候上断', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'chou-month-gui-water-formation-no-bing'),
  );
});
...
test('癸日丑月支成火局且庚辛透干时，应提升为衣食充足，不应误落到孤苦零丁层次', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'chou-month-gui-fire-formation-metal-support'),
  );
...
test('癸日丑月支成火局而庚辛不透时，应降为孤苦零丁，不应仍按见金可救论', () => {
  ...
  assert.ok(result.strategyTrace?.includes('成格层次:孤苦零丁'));
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'chou-month-gui-fire-formation-no-metal'),
  );
  assert.ok(!result.strategyTrace?.includes('成格层次:衣食充足'));
});
...
test('癸日午月庚辛透而又见壬癸时，应按金水会夏天论富贵，不应仍停留在普通夏水泛断', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wu-wei-month-gui-metal-water-summer-rich'),
  );
});
...
test('癸日午月仅金透但地支成水局时，应标记金榜挂名，不应要求壬癸也必须透干', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wu-month-gui-metal-stem-water-formation'),
  );
  assert.ok(!result.strategyTrace?.includes('成格层次:富贵永无边'));
});
...
test('癸日未月小暑后庚辛透而又有比劫扶身时，应按上半月富贵论，不应仍与下半月混断', () => {
  ...
  assert.ok(result.strategyTrace?.includes('成格层次:可云富贵'));
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wei-month-gui-xiaoshu-metal-water-rich'),
  );
});
...
test('癸日午月无水透而支只一水时，应标记一富之造，不应仍提升到金水会夏天的富贵层次', () => {
  ...
  assert.ok(
    result.matchedRules?.some(
      (rule) => rule.id === 'wu-month-gui-metal-no-visible-water-single-water-rich',
    ),
  );
  assert.ok(!result.strategyTrace?.includes('成格层次:富贵永无边'));
});
...
test('癸日午月支成炎局而无壬出干时，应标记僧道，不应仍按普通夏水富贵论', () => {
  ...
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wu-month-gui-fire-formation-no-ren-monastic'),
  );
});
...
test('癸日午月支成炎局但二壬一庚同透时，应提升为衣锦腰金，不应仍按无壬僧道处理', () => {
  ...
  assert.ok(result.strategyTrace?.includes('成格层次:衣锦腰金'));
  assert.ok(
    result.matchedRules?.some((rule) => rule.id === 'wu-month-gui-fire-formation-two-ren-one-geng'),
  );
  assert.ok(!result.strategyTrace?.includes('成格层次:定主僧道'));
});
