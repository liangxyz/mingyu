...
test('八字 AI 系统规则明确要求复核命盘中已给出的喜忌结论', () => {
  ...
  assert.match(source, /资料包里没有直接写出的额外盘面事实，不得自行补算、脑补或假定/);
...
test('八字合盘系统提示词应使用专门的双盘规则，不混入单盘要求', () => {
  ...
  assert.match(prompt.system, /资料包里没有直接写出的额外盘面事实，不得自行补算、脑补或假定/);
...
test('八字内置快捷会加入传统专项研判框架，自定义问题保持干净', () => {
  ...
  assert.match(source, /换工作问题先看当前大运、流年、流月是否对官杀、印星、食伤、财星形成新的引动/);
...
test('合盘提示词会按不同主题使用对应专项框架与任务口径', () => {
  ...
  const childrenPrompt = getCompatibilityPrompt('请分析我们的子女缘。', result1, result2, 'children');
  ...
  assert.match(parentsPrompt.user, /先说明父母议题主线，再分健康风险、照护压力、关系边界与建议展开。/);
});
