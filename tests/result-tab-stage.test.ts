...
test('结果页使用统一舞台承载八字、紫微、星盘和提示词面板，避免切换时高度抖动', () => {
...
test('结果页仅在首次访问页签后挂载对应内容，减轻首屏渲染压力', () => {
  ...
  assert.match(source, /\{mountedTabs\.astrolabe \? \(/);
...
test('结果页舞台样式会把三个面板叠放到同一区域，仅切换可见性', () => {
  ...
  assert.match(
    css,
    /\.result-tab-pane\.is-inactive\s*\{[\s\S]*visibility:\s*hidden;[\s\S]*pointer-events:\s*none;[\s\S]*\}/,
  );
});
