...
test('紫微提示词快照应输出已检测出的命盘格局', () => {
  ...
  assert.match(snapshot, /【全盘宫位索引】/);
});

test('紫微提示词上下文不再保留推荐追问字段', () => {
  const promptTypesSource = readFileSync(
    new URL('../src/lib/ziwei-prompts/types.ts', import.meta.url),
    'utf8',
  );
  const ziweiSource = readFileSync(
    new URL('../src/lib/full-chart-engine/ziwei.ts', import.meta.url),
    'utf8',
  );

  assert.doesNotMatch(promptTypesSource, /suggested_questions/);
  assert.doesNotMatch(ziweiSource, /suggested_questions/);
});

test('紫微提示词快照应输出当前报告任务，明确解读目标与边界', () => {
  const snapshot = buildZiweiReadableSnapshot({
    payload: createPayload(),
    reportContext: {
  ...
  const taskSection = snapshot.match(/【当前报告任务】([\s\S]*?)\n\n【基础信息】/)?.[1] || '';

  assert.match(snapshot, /【当前报告任务】/);
...
test('紫微提示词快照应单独输出运限落宫与当前四化飞入结构', () => {
  ...
  assert.match(snapshot, /【运限结构】/);
  assert.match(snapshot, /【运限命中摘要】/);
...
test('紫微完整提示词应补充运限解读规则', () => {
  ...
  assert.ok(prompt.indexOf('【分析对象优先级】') < prompt.indexOf('【运限解读规则】'));
  assert.ok(
    prompt.indexOf('【运限解读规则】') < prompt.indexOf('【问题】\n今年事业财运怎么判断？'),
  );
});
...
test('紫微关键证据摘要在原始证据缺少关联星曜与关联四化时应自动补全', () => {
  const payload = createPayload();
  payload.active_scope = {
    ...payload.active_scope,
    scope: 'yearly',
...
test('紫微提示词快照不应输出无意义占位的当前落宫与当前四化', () => {
  ...
  const scopeSection = snapshot.match(/【当前运限】([\s\S]*?)\n\n【命盘格局】/)?.[1] || '';
