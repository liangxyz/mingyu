...
test('非命盘占法提示词会写入各自的应期与边界规则', () => {
  ...
  for (const item of cases) {
    ...
    assert.match(prompt, /【应期与边界规则】/);
    assert.match(prompt, item.expected);
    const infoIndex = findSectionHeadingIndex(prompt, '【占卜信息】');
    const timingIndex = findSectionHeadingIndex(prompt, '【应期与边界规则】');
...
test('自定义占卜问题不强塞应期与边界规则', () => {
  ...
  assert.doesNotMatch(prompt, /【应期与边界规则】/);
});
...
test('六爻提示词会给出断卦抓手，先看用神世应动变', () => {
  ...
  assert.match(prompt, /评分口径：用神先按问题取六亲，再看是否临世应、是否发动、是否得月日触发/);
...
test('大六壬提示词会给出断课抓手，先提示发用主线再断事', () => {
  ...
  assert.match(prompt, /发用主线：亥乘贵人为发用，生扶，起因来自外部推动/);
  assert.match(prompt, /主线证据：/);
  assert.match(prompt, /主线证据：初传亥乘贵人，生扶，起因来自外部推动/);
  assert.match(prompt, /类神取用：子孙类神：主产出、解法、子女、医药与放松/);
  assert.match(prompt, /四课背景：一课亥临卯乘贵人，水生木/);
  assert.match(prompt, /三传演变：初传亥：发端与眼前触发，生扶/);
  assert.match(prompt, /空亡应期：旬空戌、亥，命中初传亥/);
  assert.match(prompt, /三传阶段：初传亥：发端与眼前触发，生扶/);
  assert.match(prompt, /中传丑：中途变化与卡点，承压/);
  assert.match(prompt, /末传寅：归结与最终落点，转合/);
  assert.match(prompt, /类神候选：子孙类神：主产出、解法、子女、医药与放松/);
  assert.match(prompt, /兄弟类神：主同辈、竞争、分财与协作牵扯/);
  assert.match(prompt, /类神只用于锁定问事主题，仍以发用、三传和四课互证为准/);
...
test('灵签提示词会给出签诗主旨、典故映射与宜进宜守抓手', () => {
  ...
  assert.match(prompt, /典故映射：已按资料包典故作现实处境类比/);
