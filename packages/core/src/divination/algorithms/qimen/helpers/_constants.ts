 ...
 */
export const palaceBranches: Record<number, string[]> = {
  1: ['子'],
  2: ['未', '申'],
  3: ['卯'],
  4: ['辰', '巳'],
...
/** 地支到宫位的映射（地盘） */
export const diPanPalaces: Record<string, number> = {
  子: 1,
  丑: 8,
  寅: 8,
  卯: 3,
...
/** 八门 → 宫位映射 */
export const doorPalaceMap: Record<string, number> = {
  休门: 1,
  生门: 8,
  伤门: 3,
  杜门: 4,
...
/** 宫位 → 八门映射 */
export const palaceDoorMap: Record<number, string> = {
  1: '休门',
  8: '生门',
  3: '伤门',
  4: '杜门',
...
/**
 * 阳遁八神顺序
 * 《奇门遁甲秘籍大全》：「阳遁顺行八神，符蛇阴合虎武地天」
 * 阳遁顺排：值符 → 螣蛇 → 太阴 → 六合 → 白虎 → 玄武 → 九地 → 九天
 */
export const yangGods = ['值符', '螣蛇', '太阴', '六合', '白虎', '玄武', '九地', '九天'];

/**
 * 阴遁八神顺序
 * 《奇门遁甲秘籍大全》：「阴遁逆行八神，符天地武虎合阴蛇」
 ...
 * 阴遁：夏至 → 大雪（共十二节气）
 */
export const jieQiJuShuMap: Record<string, { dun: '阳' | '阴'; ju: [number, number, number] }> = {
  // 阳遁（冬至阳生，顺布六仪）
  冬至: { dun: '阳', ju: [1, 7, 4] },
  小寒: { dun: '阳', ju: [2, 8, 5] },
  大寒: { dun: '阳', ju: [3, 9, 6] },
 ...
 */
export function isGenerating(source: string, target: string): boolean {
 ...
 */
export function isControlling(source: string, target: string): boolean {
 ...
 * @returns '旺' | '相' | '休' | '囚' | '死' | ''
 */
export function getElementStrengthInSeason(
  element: string,
  seasonElement: string,
): '旺' | '相' | '休' | '囚' | '死' | '' {
  if (!element || !seasonElement) return '';
  if (element === seasonElement) return '旺';
  if (isGenerating(seasonElement, element)) return '相';
  if (isGenerating(element, seasonElement)) return '休';
